...
static void pwmOutConfigKhz(pwmOutputPort_t *port, const timerHardware_t *timerHardware, uint8_t khz, uint16_t period, uint16_t value)
{
	...
	configTimeBaseKhz(timerHardware->tim, period, khz);
...
void motorInit(const motorConfig_t *motorConfig, uint16_t idlePulse, uint8_t motorCount)
{
	...
	bool useUnsyncedPwm = motorConfig->useUnsyncedPwm;			// true
...
//			printf("timerHardware->alternateFunction: %u, %s, %d\r\n", timerHardware->alternateFunction, __FUNCTION__, __LINE__);
			...
			pwmOutConfigMhz(&motors[motorIndex], timerHardware, timerMhzCounter, periodHz, idlePulse);
		}else {
...
void ledTimerInit(const LedTimerConfig_t *ledTimerConfig, uint16_t idlePulse, uint8_t ledCount, uint32_t KhzGain)
{
	...
	for (int ledIndex = 0; ledIndex < LED_NUMBER; ledIndex++) {
