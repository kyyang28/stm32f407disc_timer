...
void SysTick_Handler(void)
{
	...
	for ( uint8_t __ToDo = __basepriSetMemRetVal(NVIC_PRIO_MAX); __ToDo ; __ToDo = 0 ) {
	
	/* It seems __cleanup__ doesn't get invoked in Keil IDE */
	//for (uint8_t __basepri_save __attribute__ ((__cleanup__(__basepriRestoreMem))) = __get_BASEPRI(), __ToDo = __basepriSetMemRetVal(NVIC_PRIO_MAX); __ToDo ; __ToDo = 0) {
		sysTickUptime++;
		sysTickPending = 0;
		(void)(SysTick->CTRL);
